...
# Load sentences only once per session
if 'sentences' not in st.session_state:
    eligible = df[df['Usage_Count'] < 2]
    if len(eligible) < 10:
...
if not st.session_state.submitted:
    if st.button("সাবমিট / Submit"):
        ...
        if has_translation:
            ...
            st.rerun()  # ← CRITICAL: Force rerun immediately to show the else branch
        else:
